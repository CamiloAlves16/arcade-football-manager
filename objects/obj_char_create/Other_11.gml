/// @description FIRST DAY THINK (NEEDS TO REMOVE COACH THINK WHEN COPIED TO OBJ_CAREER_MODE)
// ALSO MAKES SURE OFFERS AREN'T MADE FOR PLAYERS IN C10 TEAM

for (var i = 1; i < array_length(global.coach_list); ++i) {
		
	coach = global.coach_list[i];
...
for (var i = 0; i < array_length(global.team_list); ++i) {
	...
	if (global.team_list[i] != global.c10_team) {
		...
		minimum_squad_level = lerp(8, 15, (variable_global_get(global.team_list[i] + "_reputation") - 1) / 4);
		...
		if (array_length(needs) < 1) { // MY ROSTER FULL LET ME SEE WHAT I DO WITH THE OTHER PLAYERS			
			for (var j = 0; j < array_length(players_on_the_squad); ++j) {
				...
				} else if (!array_contains(global.player_transfers, players_on_the_squad[j]) && variable_global_get(players_on_the_squad[j] + "_og_team") == global.team_list[i] && variable_global_get(players_on_the_squad[j] + "_potential") - variable_global_get(players_on_the_squad[j] + "_ca") <= 0) {
		...
		for (var j = 0; j < array_length(needs); ++j) {
			...
			if (global.week < global.max_weeks/2) {
				...
				for (var k = 0; k < array_length(global.player_list); ++k) {
					...
					if (variable_global_get(global.player_list[k] + "_transfer_eligible") && !array_contains(variable_global_get(global.player_list[k] + "_offers_received"), global.team_list[i]) && target_og_team != global.team_list[i] && target_og_team != global.c10_team) {
						...
						if (array_contains(global.player_freeagent, global.player_list[k]) && variable_global_get(global.player_list[k] + "_salary") <= salary_per_need) {
							transfer_fee = 0;
							is_target = true;
						} else if (array_contains(global.player_loans, global.player_list[k]) && calculate_salary(global.player_list[k], false) <= salary_per_need) {
							transfer_fee = 0;
							is_target = true;
						} else if (transfer_fee <= budget_per_need && calculate_salary(global.player_list[k], false) <= salary_per_need) {
							is_target = true;
						}
